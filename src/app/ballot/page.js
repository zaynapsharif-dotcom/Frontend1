...
export default function BallotPage() {
  ...
  const hasAlreadyVoted = !!auth.voter?.hasVoted;
  ...
  // Guard: prevent already-voted users from opening the ballot UI
  useEffect(() => {
    if (!auth.isReady) return;
  ...
  }, [auth.isReady, auth.isLoggedIn, hasAlreadyVoted, router]);
  ...
  }, [auth.isReady, auth.isLoggedIn, auth.token, hasAlreadyVoted]);
  ...
  if (!auth.isReady || hasAlreadyVoted) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm font-semibold text-slate-500 animate-pulse tracking-wide">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-slate-400">
            ...
            <span className="flex items-center gap-1">
              <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-200 bg-white text-[10px]">
                2
              </span>
              Confirm Vote
            </span>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200/60 pb-8">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 text-sm font-medium text-slate-500">
                ...
                <span className="hidden sm:inline text-slate-400">
                  Select one candidate to proceed
                </span>
              </div>
            </div>
            ...
            >
              <div className="relative">
                <svg
                  className="h-4 w-4 text-slate-400 transition-transform duration-700 group-hover:rotate-180 group-hover:text-emerald-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            {[1, 2].map((i) => (
              <div key={i} className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {[1, 2, 3].map((j) => (
                    <div key={j} className="h-24 rounded-xl bg-slate-50 animate-pulse" />
                  ))}
                </div>
              </div>
            ))}
        ...
        {!loading && sortedParties.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 py-24 text-center">
            ...
            <p className="text-slate-500 max-w-sm mt-2 leading-relaxed">
              The election configuration currently has no active parties or candidates listed.
            </p>
          </div>
        ) : null}
            ...
            {sortedParties.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className="absolute top-0 inset-x-0 h-32 bg-slate-50 opacity-50 pointer-events-none"
                    style={{
                      backgroundImage:
                  ...
                  <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 md:p-8">
                    ...
                    {partyStatus && (
                      <div
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border shadow-sm ${
                        ...
                        {activeCandidates.map((c) => {
                          ...
                          return (
                            <button
                              ...
                              className="group/card relative flex flex-col items-start gap-4 rounded-xl border border-slate-200 bg-white p-5 text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400 hover:shadow-lg hover:shadow-emerald-500/10 hover:ring-1 hover:ring-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:border-slate-200 disabled:hover:shadow-sm disabled:hover:ring-0"
                            >
                              <div className="flex w-full items-start justify-between">
                                ...
                                <div className="h-5 w-5 rounded-full border-2 border-slate-200 transition-colors group-hover/card:border-emerald-500 group-hover/card:bg-emerald-500 flex items-center justify-center">
                                  <svg
                                    className="h-3 w-3 text-white opacity-0 group-hover/card:opacity-100 transition-opacity"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                  >
