...
export default function AdminDashboardPage() {
  ...
  async function runAction(path, options = {}) {
    ...
    if (requireFinalizeText) {
      const typed = window.prompt(
        `Final confirmation required.\n\nType FINALIZE to finalize election "${ELECTION_ID}".`
      );

      if (typed !== "FINALIZE") {
        setBanner({
          type: "error",
          message: "Finalize cancelled. Confirmation text did not match.",
        });
        return;
      }
    }
    ...
    try {
      ...
      setBanner({ type: "success", message: `Action succeeded: ${label}` });
      await fetchTotals();
    } catch (e) {
      const err = handleError(e);
      setBanner({ type: "error", message: err.message });
    ...
    <main className="min-h-screen w-full bg-slate-50/50 p-6 md:p-10 font-sans text-slate-900 animate-fade-in-up">
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60">
          ...
          >
            <svg
              className={`w-4 h-4 text-slate-400 transition-transform duration-700 group-hover:text-emerald-500 ${
        ...
        {banner && (
          <div
            className={`rounded-xl border p-4 shadow-sm flex items-start gap-3 animate-in fade-in slide-in-from-top-2 ${
              banner.type === "error"
          ...
          >
            <div
              className={`shrink-0 rounded-full p-1 ${
                banner.type === "error" ? "bg-red-100 text-red-600" : "bg-emerald-100 text-emerald-600"
              }`}
            >
              {banner.type === "error" ? (
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1 pl-3.5">
                  Real-time participation overview.
                </p>
              </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
            ...
            <div className="mt-6 space-y-3">
              <button
                onClick={() =>
                  runAction("/admin/open", {
                    label: "Open Election",
                    confirmMessage:
                      `Open election "${ELECTION_ID}"?\n\n` +
                      "This will allow eligible voters to issue voting tokens and submit votes.",
                  })
                }
                disabled={loading}
              ...
              <button
                onClick={() =>
                  runAction("/admin/close", {
                    label: "Close Election",
                    confirmMessage:
                      `Close election "${ELECTION_ID}"?\n\n` +
                      "This should stop voters from issuing new voting tokens or submitting votes.",
                  })
                }
                disabled={loading}
              ...
              <button
                onClick={() =>
                  runAction("/admin/finalize", {
                    label: "Finalize Results",
                    confirmMessage:
                      `Finalize election "${ELECTION_ID}"?\n\n` +
                      "This will compute final totals, store a snapshot, and mark the election as finalized. Only continue if voting is complete.",
                    requireFinalizeText: true,
                  })
                }
                disabled={loading}
                ...
                <div className="space-y-3">
                  {perParty.map((p, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/30 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all group"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="text-sm font-bold text-slate-700 group-hover:text-indigo-900 transition-colors">
                            {name}
                          </div>
                        </div>
